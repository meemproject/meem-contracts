...
contract MeemFacet {
	...
	{
		require(_owner != address(0), "MeemFacet: _owner can't be address(0");
	...
	{
		require(_index < s.tokenIds.length, 'MeemFacet: index beyond supply');
		...
		returns (uint256 tokenId_)
	{
		require(
			...
			'MeemFacet: index beyond owner balance'
		);
	...
	function internalTransferFrom(
		...
		require(_to != address(0), "MeemFacet: Can't transfer to 0 address");
		require(_from != address(0), "MeemFacet: _from can't be 0 address");
		require(
			_from == s.meems[_tokenId].owner,
			'MeemFacet: _from is not owner, transfer failed'
		);
		require(
			_sender == _from ||
				s.operators[_from][_sender] ||
				_sender == s.approved[_tokenId],
			'MeemFacet: Not owner or approved to transfer'
		);
	...
	function tokenURI(uint256 _tokenId) external pure returns (string memory) {
		return
			LibStrings.strWithUint(
				'https://aavegotchi.com/metadata/aavegotchis/',
				_tokenId
			); //Here is your URL!
	}
}
